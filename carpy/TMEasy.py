...
class Tire:
    ...
    def __tire_offset__(self, Sy):
        ...
        if Sy < self.SyE:  # 4th - order polynomial 0 <= Sy < SyE
            ...
            n2L = self.n2L0 * (
                1 + (Sy / (self.Sy0 * self.SyE)) ** 2 * (a + Sy * (b + Sy * c))
            )
        else:
            n2L = 0  # Full sliding
        return n2L
    ...
    def __tire_combined_forces__(self, S, df0, Fm, Sm, Fs, Ss):
        ...
        if df0 > 0:
            ...
            return print("df0 must be greater than 0. Please insert a valid df0.")
        ...
        if S > 0 and Smloc > 0:
            if S > Ssloc:  # Full sliding
                ...
                S < Smloc and Fm > 0
            ):  # Full adhesion. The force varies linearly with the slip
                p = df0 * Smloc / Fm - 2
                Sn = S / Smloc
                dF = df0 / (1 + (Sn + p) * Sn)
                F = dF * S
    ...
    def calculateSlip(self, Vx, Vy, delta, gamma, omega_wheel, Wn, Fz):
        ...
        Vn = 0.01
        Rd = self.lamb * self.tire_radius + (1 - self.lamb) * (
            self.tire_radius - Fz / self.cz
        )
    ...
    def get_tire_forces(self, Sx, Sy, Sg, Sb, Fz):
        ...
        df0 = np.sqrt(
            (self.dfx0 * self.hsxn * cphi) ** 2 + (self.dfy0 * self.hsyn * sphi) ** 2
        )
        Fm = np.sqrt((self.Fxm * cphi) ** 2 + (self.Fym * sphi) ** 2)
        Sm = np.sqrt(
            (self.Sxm / self.hsxn * cphi) ** 2 + (self.Sym / self.hsyn * sphi) ** 2
        )
        Fs = np.sqrt((self.Fxs * cphi) ** 2 + (self.Fys * sphi) ** 2)
        Ss = np.sqrt(
            (self.Sxs / self.hsxn * cphi) ** 2 + (self.Sys / self.hsyn * sphi) ** 2
        )
        ...
        Mz = (
            -N * Fy
            + (L / 3) * self.__tire_combined_forces__(Sb, df0, Fm, Sm, Fs, Ss)[0]
        )  # Rb = L/3
    ...
    def all_info(self, Fz=16000, camber_slip=0, bore_slip=0, matplotlib=True):
        ...
        for slipy in sy:
            Fx = [
                self.get_tire_forces(Sx[i], slipy, camber_slip, bore_slip, Fz)[0]
            ...
            forces.append(
                Function(
                    Sx,
                    Fx,
                    "Slip longitudinal (sx)",
                    "Força longitudinal (N)",
                    name="Sy={:.2F}, Sg={:.2F}".format(slipy, camber_slip),
                )
            )
        ...
        for slipx in sx:
            Fy = [
                self.get_tire_forces(slipx, Sy[i], camber_slip, bore_slip, Fz)[1]
            ...
            forces.append(
                Function(
                    ...
                    "Slip Lateral (sy)",
                    "Força Lateral (N)",
                    name="Sx={:.2F}, Sg={:.2F}".format(slipx, camber_slip),
                )
            )
        print(
            "Coenering stiffness = {:.1f}N/৹".format(
                forces[0].derivate(0, np.deg2rad(1)) * np.deg2rad(1)
            )
        )
        ...
        for slipx in sx:
            Mz = [
                self.get_tire_forces(slipx, Sy[i], camber_slip, bore_slip, Fz)[2]
                for i in range(len(Sy))
            ]
            forces.append(
                Function(
